...
export interface PerBookContext {
  visibleText?: string;
...
export interface ChatAgentStartBody {
  userId: string;
  sessionId: string;
  /** Legacy single-book id. Used as the primary when `bookIds` is absent. */
  bookId?: string;
  /** Selected books for the conversation; `bookIds[0]` is the primary. */
  bookIds?: string[];
  message: UIMessage;
  visibleText?: string;
  currentChapterIndex?: number;
...
export async function createChatStreamResponse(
  ...
  // The primary always comes first and the set is de-duplicated.
