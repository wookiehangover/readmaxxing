...
Ebook/PDF reader web app: React Router v7 (framework mode) + TypeScript, Tailwind v4, shadcn/ui (Base UI), `ReadingShell` + `LibraryFrame`, `@readmaxxing/epub-successor` + pdfjs, TipTap notebooks, Themis, Postgres (`pg`), WebAuthn passkeys, Vercel Blob, local-first sync.
